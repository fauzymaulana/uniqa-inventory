...
<div class="card mb-4">
    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-1">
            ...
            <form action="{{ route('admin.invitation.kategori.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Hapus kategori {{ $category->name }}? Semua produk di kategori ini akan ikut terhapus.')">
                @csrf
                @method('DELETE')
                    ...
                    <div class="col-sm-6 col-md-4 col-lg-3">
                        <div class="card h-100 border">
                            ...
                            <div class="card-footer bg-transparent border-top-0 d-flex gap-2">
                                <a href="{{ route('admin.invitation.edit', $product->id) }}" class="btn btn-sm btn-outline-primary flex-fill">
                                    <i class="fas fa-edit"></i> Edit
                                </a>
                                <form action="{{ route('admin.invitation.destroy', $product->id) }}" method="POST" class="flex-fill" onsubmit="return confirm('Hapus produk ini?')">
                                    @csrf
                                    @method('DELETE')
...
<div class="modal fade" id="editCategoryModal{{ $category->id }}" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('admin.invitation.kategori.update', $category->id) }}" method="POST">
                @csrf
                @method('PUT')
