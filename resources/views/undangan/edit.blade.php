...
@section('title', 'Edit Produk Undangan')

@section('content')
<div class="row">
    <div class="col-12">
        <h2><i class="fas fa-edit"></i> Edit Produk Undangan</h2>
        <hr>
    </div>
</div>
                ...
                <form action="{{ route('undangan.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="invitation_category_id" class="form-select @error('invitation_category_id') is-invalid @enderror" required>
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ $product->invitation_category_id == $cat->id ? 'selected' : '' }}>
                                    {{ $cat->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        @if($product->thumbnail)
                            <div class="mb-2">
                                <img src="{{ asset('storage/undangan/' . $product->thumbnail) }}" alt="Current Thumbnail"
                                     style="max-height:150px;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                                <p class="text-muted small mt-1">Thumbnail saat ini. Upload baru untuk mengganti.</p>
                            </div>
                        @endif
                        ...
                        <div id="thumbnailPreview" class="mt-2" style="display:none;">
                            <img id="previewImg" src="" alt="Preview" style="max-height:200px;border-radius:8px;">
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        @if($product->video_demo)
                            <div class="mb-2">
                                <video controls style="max-height:150px;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,0.1);max-width:100%;">
                                    <source src="{{ asset('storage/undangan/videos/' . $product->video_demo) }}" type="video/mp4">
                                </video>
                                <p class="text-muted small mt-1">Video saat ini. Upload baru untuk mengganti.</p>
                            </div>
                        @endif
                        <input type="file" name="video_demo" class="form-control @error('video_demo') is-invalid @enderror"
                               accept="video/mp4" id="videoInput">
                        <div class="form-text">Format: MP4. Maks 20MB.</div>
                        @error('video_demo')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <div id="videoPreview" class="mt-2" style="display:none;">
                            <video id="previewVideo" controls style="max-height:200px;border-radius:8px;max-width:100%;"></video>
                        </div>
                    </div>
